...
namespace UniversalTunTapDriver
{
    public class TunTapDevice
    {
        ...
        public bool SetDHCPState(int iState=1)
        {
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
                uint Length = 0;
                IntPtr cconfig = Marshal.AllocHGlobal(4);
                Marshal.WriteInt32(cconfig, (int)iState);
                return DeviceIoControl((IntPtr)DeviceHandle, CTL_CODE(FILE_DEVICE_UNKNOWN, TAP_WIN_IOCTL_CONFIG_DHCP_SET_OPT, METHOD_BUFFERED, FILE_ANY_ACCESS), cconfig, 4, cconfig, 4, ref Length, IntPtr.Zero);
            }
            else if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
            {
                //On linux platform this operation must be done manually
